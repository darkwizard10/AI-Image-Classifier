...
def main():
    ...
    uf=st.file_uploader("Choose an image.....",type=["jpg","png","jpeg"])

    if uf is not None:
        image=st.image(
            uf,caption="Uploaded Image",use_container_width=True
        )
        ...
        if btn:
            with st.spinner("Analyzing the Image....."):
                image=Image.open(uf)
                predictions=clsfy_img(model,image)
